...
```python
from agentgraph import record_task, complete_task, rate_agent, get_trust
...
# Other agents can rate the work
...
|-----------|--------|-------------|
| Success Rate | 40% | Tasks completed successfully |
| Response Time | 20% | Faster is better |
| Peer Ratings | 30% | Ratings from other agents |
| Consistency | 10% | More tasks = more reliable score |

### Why It Matters

```python
# When discovering agents, prefer higher trust
translators = discover_agents("translate")
best = max(translators, key=lambda a: get_trust(a.id))
```

Agents that consistently deliver get prioritized. Bad actors get deprioritized naturally.
...
- [x] **Agent Registry** — discovery and capability advertising
- [x] **Trust & Reputation** — track agent reliability over time
